...
export const madeInChinaAdapter: ScraperAdapter = {
  source: "madeinchina",
  supportsApi: true,
  supportsBrowser: true,
  async scrape(query: ScrapeQuery, ctx: AdapterContext): Promise<NormalizedProduct[]> {
    const baseUrl = config.providers.madeinchina.baseUrl;
    const apiKey = config.providers.madeinchina.apiKey;

    if (!baseUrl || !apiKey) {
      throw new Error("Made-in-China API not configured. Set MADEINCHINA_API_BASE and MADEINCHINA_API_KEY.");
    }
    ...
    const response = await withRetry(
      async () => {
        ...
        try {
          ...
          if (!res.ok) throw new Error(`Made-in-China API error: ${res.status}`);
          return res.json();
        } finally {
          clearTimeout(timer);
        }
