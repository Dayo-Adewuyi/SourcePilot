...
var scrapeSuppliers = (nodeRuntime) => {
  const config = nodeRuntime.config;
  const httpClient = new ClientCapability2;
  const categories = ["rice", "beans", "egg"];
  const allProductHashes = [];
  const allEntries = [];
  const entryCounts = [];
  const targetCategories = categories.slice(0, config.maxProductsPerScan);
  ...
  const batchResponse = httpClient.sendRequest(nodeRuntime, {
  ...
  if (batchResponse.statusCode !== 200) {
    nodeRuntime.log(`[PriceScanner] Batch scrape failed: HTTP ${batchResponse.statusCode}`);
    return { productHashes: [], allEntries: [], entryCounts: [] };
  }
  const batchData = JSON.parse(new TextDecoder().decode(batchResponse.body));
  for (const category of targetCategories) {
    ...
    if (aiResponse.statusCode !== 200) {
      nodeRuntime.log(`[PriceScanner] AI analysis failed for "${category}": HTTP ${aiResponse.statusCode}`);
      continue;
    }
    const analyzed = JSON.parse(new TextDecoder().decode(aiResponse.body));
...
var onCronTrigger = (runtime2, _trigger) => {
  ...
  runtime2.log("[PriceScanner] Starting scan cycle...");
  ...
  const scanResult = runtime2.runInNodeMode(scrapeSuppliers, consensusIdenticalAggregation())().result();
  if (scanResult.productHashes.length === 0) {
    runtime2.log("[PriceScanner] No products to update. Exiting.");
    return "NoUpdates";
  }
  runtime2.log(`[PriceScanner] Found deals for ${scanResult.productHashes.length} products`);
  ...
  const callData = encodeFunctionData({
    ...
    args: [
      scanResult.productHashes.map((h) => h),
      contractEntries,
  ...
  const writeResult = evmClient.writeReport(runtime2, { receiver: config.priceOracleAddress, report: report2 }).result();
  runtime2.log(`[PriceScanner] PriceOracle updated. TX: ${writeResult.txStatus}`);
