...
var checkAllDeliveries = (nodeRuntime, shippedDealIds, trackingKey) => {
  const config = nodeRuntime.config;
  const httpClient = new cre.capabilities.HTTPClient;
  const results = [];
  for (const dealId of shippedDealIds) {
    const trackingResponse = httpClient.sendRequest(nodeRuntime, {
    ...
    const tracking = JSON.parse(new TextDecoder().decode(trackingResponse.body));
...
var onCronTrigger = (runtime2, _trigger) => {
  const config = runtime2.config;
  runtime2.log("[DeliveryVerifier] Starting delivery check cycle...");
  ...
  const nextDealId = Number(decodeFunctionResult({
    abi: EscrowVaultAbi,
    functionName: "nextDealId",
    data: bytesToHex(nextDealResult.data)
  }));
  const shippedDealIds = [];
  const dealAgentMap = new Map;
  ...
  for (let dealId = scanStart;dealId < nextDealId; dealId++) {
    ...
    const statusResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: config.escrowVaultAddress,
        data: statusCallData
    ...
    const status = Number(decodeFunctionResult({
      abi: EscrowVaultAbi,
      functionName: "getDealStatus",
    ...
    if (status === DealStatus.Shipped) {
      ...
      const dealDetailResult = evmClient.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddress,
          to: config.escrowVaultAddress,
          data: dealCallData
      ...
      const dealDetail = decodeFunctionResult({
        abi: EscrowVaultAbi,
        functionName: "getDeal",
        data: bytesToHex(dealDetailResult.data)
      });
      dealAgentMap.set(dealId, Number(dealDetail.agentId));
    }
  }
  if (shippedDealIds.length === 0) {
    runtime2.log("[DeliveryVerifier] No shipped deals to verify.");
    return "NoShippedDeals";
  }
  shippedDealIds.sort((a, b) => a - b);
  runtime2.log(`[DeliveryVerifier] Checking ${shippedDealIds.length} shipped deals`);
  const trackingKey = runtime2.getSecret({ id: "TRACKING_API_KEY" }).result().value;
  const trackingResults = runtime2.runInNodeMode((nodeRuntime) => checkAllDeliveries(nodeRuntime, shippedDealIds, trackingKey), consensusIdenticalAggregation())().result();
  let deliveredCount = 0;
  let anomalyCount = 0;
  for (const result of trackingResults.results) {
    if (result.isAnomaly) {
      anomalyCount++;
      runtime2.log(`[DeliveryVerifier] ANOMALY deal #${result.dealId}: ${result.anomalyReason}`);
    }
    if (result.isDelivered) {
      ...
      const deliveryReport = runtime2.report({
      ...
      runtime2.log(`[DeliveryVerifier] Deal #${result.dealId} delivered → escrow released. TX: ${writeResult.txStatus}`);
      ...
      if (agentId) {
        ...
        const statsReport = runtime2.report({
  ...
  runtime2.log(`[DeliveryVerifier] Cycle complete. ${summary}`);
  return summary;
};
