...
var evmConfigSchema = exports_external2.object({
  ...
  isTestnet: exports_external2.boolean()
});
var priceScannerConfigSchema = exports_external2.object({
  schedule: exports_external2.string(),
  scraperBaseUrl: exports_external2.string(),
  aiServiceUrl: exports_external2.string(),
  priceOracleAddress: exports_external2.string(),
  agentRegistryAddress: exports_external2.string(),
  evm: evmConfigSchema,
  maxProductsPerScan: exports_external2.number(),
  maxEntriesPerProduct: exports_external2.number()
});
var dealExecutorConfigSchema = exports_external2.object({
  scraperBaseUrl: exports_external2.string(),
  aiServiceUrl: exports_external2.string(),
  escrowVaultAddress: exports_external2.string(),
  purchaseOrderAddress: exports_external2.string(),
  agentRegistryAddress: exports_external2.string(),
  logisticsApiUrl: exports_external2.string(),
  evm: evmConfigSchema
});
var deliveryVerifierConfigSchema = exports_external2.object({
  schedule: exports_external2.string(),
  trackingApiUrl: exports_external2.string(),
  escrowVaultAddress: exports_external2.string(),
  purchaseOrderAddress: exports_external2.string(),
  agentRegistryAddress: exports_external2.string(),
  evm: evmConfigSchema,
  maxDealsPerScan: exports_external2.number()
});
var verifyAndBook = (nodeRuntime, dealMetadata) => {
  ...
  const stockResponse = httpClient.sendRequest(nodeRuntime, {
  ...
  const products = JSON.parse(new TextDecoder().decode(stockResponse.body));
  ...
  const riskResponse = httpClient.sendRequest(nodeRuntime, {
  ...
  if (riskResponse.statusCode === 200) {
    const riskData = JSON.parse(new TextDecoder().decode(riskResponse.body));
  ...
  const bookingResponse = httpClient.sendRequest(nodeRuntime, {
  ...
  if (bookingResponse.statusCode === 200 || bookingResponse.statusCode === 201) {
    const bookingData = JSON.parse(new TextDecoder().decode(bookingResponse.body));
...
var onDealCreated = (runtime2) => {
  const config = runtime2.config;
  runtime2.log("[DealExecutor] DealCreated event received");
  ...
  const nextDealResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: config.escrowVaultAddress,
  ...
  const nextDealId = decodeFunctionResult({
    abi: EscrowVaultAbi,
    functionName: "nextDealId",
    data: bytesToHex(nextDealResult.data)
  });
  const dealId = Number(nextDealId) - 1;
  if (dealId < 1) {
    runtime2.log("[DealExecutor] No deals found");
    return "NoDeal";
  }
  ...
  const dealResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: config.escrowVaultAddress,
  ...
  const deal = decodeFunctionResult({
    abi: EscrowVaultAbi,
    functionName: "getDeal",
    data: bytesToHex(dealResult.data)
  });
  if (Number(deal.status) !== DealStatus.Locked) {
    runtime2.log(`[DealExecutor] Deal ${dealId} is not Locked (status=${deal.status}). Skipping.`);
    return "SkippedNotLocked";
  }
  runtime2.log(`[DealExecutor] Processing deal #${dealId}: ${deal.amount} USDC`);
  const logisticsKey = runtime2.getSecret({ id: "LOGISTICS_API_KEY" }).result().value;
  const scraperKey = runtime2.getSecret({ id: "SCRAPER_API_KEY" }).result().value;
  const checkResult = runtime2.runInNodeMode((nodeRuntime) => verifyAndBook(nodeRuntime, {
    dealId,
  ...
  if (!checkResult.available || checkResult.riskScore > 7500) {
    runtime2.log(`[DealExecutor] Deal #${dealId} rejected: ${checkResult.riskReason}`);
  ...
  const confirmReport = runtime2.report({
    encodedPayload: hexToBase64(confirmCallData),
    encoderName: "evm",
    signingAlgo: "ecdsa",
    hashingAlgo: "keccak256"
  }).result();
  const confirmResult = evmClient.writeReport(runtime2, {
    receiver: config.escrowVaultAddress,
    report: confirmReport
  }).result();
  runtime2.log(`[DealExecutor] Deal #${dealId} confirmed. TX: ${confirmResult.txStatus}`);
  if (checkResult.bookingRef) {
    ...
    const shipReport = runtime2.report({
    ...
    runtime2.log(`[DealExecutor] Deal #${dealId} shipped. Tracking: ${checkResult.bookingRef}`);
  }
